...
class TestProductRoutes(TestCase):
    ...
    def _create_products(self, count: int = 1) -> list:
        ...
        for _ in range(count):
            ...
            self.assertEqual(
                ...
                f"Could not create test product: {response.status_code} - {response.data}"
            )
    ...
    def test_create_product(self):
        ...
        new_product = response.get_json()
        self.assertIsNotNone(new_product, "POST response does not contain valid JSON")
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        new_product = response.get_json()
        # Security validation for second request
        self.assertIsNotNone(new_product, f"Could not retrieve product from {location}")
        self.assertEqual(new_product["name"], test_product.name)
        self.assertEqual(new_product["description"], test_product.description)
        self.assertEqual(Decimal(new_product["price"]), test_product.price)
        self.assertEqual(new_product["available"], test_product.available)
    ...
    def get_product_count(self):
        ...
        response = self.client.get(BASE_URL, headers={"Content-Type": "application/json"})
        if response.status_code != 200:
            logging.error("Error %s: %s", response.status_code, response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.get_json()
    ...
    def test_get_product(self):
        ...
        self.assertIsNotNone(data, "El cuerpo de la respuesta está vacío")
        # Comparar los datos obtenidos con el objeto original
        self.assertEqual(data["id"], test_product.id)
        self.assertEqual(data["name"], test_product.name)
        # Si tienes otros campos importantes, agrégalos para mayor cobertura:
    ...
    def test_get_product_not_found(self):
        ...
        # get the data from resp.get_json()
        data = response.get_json()
        # assert that message "was not found" is shown
    ...
    def test_delete_product(self):
        ...
        # assert that the resp.status_code is status.HTTP_204_NO_CONTEN
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        # Verify the response data is empty
